...
public class WaveFunctionCollapse : MonoBehaviour
{
    ...
    GreedBestFirstSearch gd;
    public ParticleSystem particle;
    float rate;
    public List<Material> skybox;
    Light gamelight;
    public void StartPoint(int startIndex)
    {
        wayIn.Add(startIndex);
    }
    public void EndPoint(int endIndex)
    {
        gateOut.Add(endIndex);
    }
    void RandomSkyBox()
    {
    ...
    void Awake()
    {
        ...
        gd = GetComponent<GreedBestFirstSearch>();
    }
    void InitializeGrid()
    {
    ...
    void ShowWayPoint(List<Cell> cell)
    {
        if(dimensions >= 15 && secondWaypoint == false)
        {
            ...
            foreach(Cell t in cell)
            {
                ...
                if(y >= dimensions - 1)
                {
                   Tile gameObject = Instantiate(way[2],t.transform.position,transform.rotation);
                   int index = cell.IndexOf(selectedCell);
                   EndPoint(index);
                   instantiateObject.Add(gameObject);
                }else if(y == 0)
                {
                    Tile gameObject = Instantiate(way[1],t.transform.position,transform.rotation);
                    int index = cell.IndexOf(selectedCell);
                    StartPoint(index);
                    instantiateObject.Add(gameObject);
                }else{
                    Tile gameObject = Instantiate(way[0],t.transform.position,transform.rotation);
                    instantiateObject.Add(gameObject);
                }
    ...
    void CollapseCell (List<Cell> tempGrid)
    {
        ...
        Tile gameObject = Instantiate(collapseCell.tileOptions[0], collapseCell.transform.position, collapseCell.tileOptions[0].transform.rotation);
        instantiateObject.Add(gameObject);
        UpdateGeneration();
        }   
    }
